...
console.log('🔍 Проверка переменных окружения для KattyFit (v0)...\n');
...
if (hasErrors) {
  ...
  console.log('\n🔧 Что делать:');
  console.log('1. Откройте ваш проект в v0.dev');
  console.log('2. Перейдите в Settings → Environment Variables');
  console.log('3. Добавьте отсутствующие переменные:');
  missing.forEach(varName => {
    console.log(`   - ${varName}`);
  });
  console.log('4. Сохраните изменения и перезапустите проект');
  console.log('5. См. подробную инструкцию в V0_ENV_SETUP.md');
  process.exit(1);
} else {
